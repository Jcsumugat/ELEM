...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'login') {
        ...
        if ($user && verifyPassword($password, $user['password'])) {
            ...
            $redirectPage = ($isElementary) ? 'elementary_dashboard.php' : 'dashboard.php';
    ...
    elseif ($action === 'logout') {
        ...
        $system = $_SESSION['system'] ?? 'college';
    ...
    elseif ($action === 'register') {
        ...
        $stmt = $conn->prepare("INSERT INTO users (username, email, fullname, password, role, is_active) VALUES (?, ?, ?, ?, ?, 1)");
